...
if __name__ == "__main__":
    ...
    for n in range(n_folds):
        ...
        #  a ton of changes.
        train = df.iloc[train_indices, :].to_csv(os.path.join("data", TRAIN_NAME), header=False, index=False)
        test = df.iloc[test_indices, :].to_csv(os.path.join("data", TEST_NAME), header=False, index=False)
        ...
        s_prs, accuracy, mae, s2, s2pond, rat, lkh, theta, eta = mmsbm.postprocess(
            return_dict
        )
